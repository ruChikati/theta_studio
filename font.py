...
class Font:
    ...
    def __init__(self, file_path, order_path, bar_colour=(128, 128, 128), colourkey=(0, 0, 0)):
        ...
        if len(set(self.order).difference('!-.0123456789:;?ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz')) > 0:
            raise ForbiddenCharacterError(''.join(set(self.order).difference('!-.0123456789:;?ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz')))
        ...
        for i in range(1, len(self.distances)):
            img = clip(self.fnt_img, self.distances[i - 1], 0, self.distances[i] - self.distances[i - 1], self.fnt_img.get_height())
