...
class Input:
    ...
    def get(self) -> list[Event]:
        ...
        for event in pygame.event.get():
            match event.type:
                case pygame.KEYDOWN:
                    if len([cache_event for t, cache_event in zip(self.cache[0], self.cache[1]) if t >= cur_time - LAG_TIME and cache_event == Event(KEYDOWN, {'key': event.key, 'unicode': event.unicode, 'mod': event.mod, 'scancode': event.scancode})]):
                        ...
                        if event.key not in self._held_keys['keys']:
                            ...
                            self._hold_start['keys'][event.key] = cur_time  # must always set this!!!
                    else:
                        return_list.append(Event(KEYDOWN, {'key': event.key, 'unicode': event.unicode, 'mod': event.mod, 'scancode': event.scancode}))
                        self.cache[1].append(Event(KEYDOWN, {'key': event.key, 'unicode': event.unicode, 'mod': event.mod, 'scancode': event.scancode}))
                        self.cache[0].append(cur_time)
                ...
                case pygame.MOUSEBUTTONDOWN:
                    if len([cache_event for t, cache_event in zip(self.cache[0], self.cache[1]) if t >= cur_time - LAG_TIME and cache_event == Event(MOUSEDOWN, {'pos': event.pos, 'button': event.button})]):
                ...
                case pygame.MOUSEBUTTONUP:
                    if len([cache_event for t, cache_event in zip(self.cache[0], self.cache[1]) if t >= cur_time - LAG_TIME and cache_event == Event(MOUSEUP, {'pos': event.pos, 'button': event.button})]):
                        ...
                        self._held_keys['mouse'].remove(event.button)
                case pygame.KEYUP:
                    if len([cache_event for t, cache_event in zip(self.cache[0], self.cache[1]) if t >= cur_time - LAG_TIME and cache_event == Event(KEYUP, {'key': event.key, 'mod': event.mod})]):
                ...
                case pygame.FINGERUP:
                    if len([cache_event for t, cache_event in zip(self.cache[0], self.cache[1]) if t >= cur_time - LAG_TIME and cache_event == Event(FINGERUP, {'touch_id': event.touch_id, 'finger_id': event.finger_id, 'x': event.x, 'y': event.y, 'dx': event.dx, 'dy': event.dy})]):
                ...
                case pygame.FINGERDOWN:
                    if len([cache_event for t, cache_event in zip(self.cache[0], self.cache[1]) if t >= cur_time - LAG_TIME and cache_event == Event(FINGERDOWN, {'touch_id': event.touch_id, 'finger_id': event.finger_id, 'x': event.x, 'y': event.y, 'dx': event.dx, 'dy': event.dy})]):
        ...
        for key in self._held_keys['keys']:
            self.posted_events.append(Event(KEYHOLD, {'key': key, 'time': cur_time - self._hold_start['keys'][key]}))
        for button in self._held_keys['mouse']:
            self.posted_events.append(Event(MOUSEHOLD, {'button': button, 'time': cur_time - self._hold_start['mouse'][button]}))
